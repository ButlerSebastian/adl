...
exports.ADLHoverProvider = void 0;
const vscode = __importStar(require("vscode"));
class ADLHoverProvider {
    constructor() {
    ...
    getTypeHover(typeName, document) {
        ...
        if (match) {
            ...
            const markdown = new vscode.MarkdownString(`**Type**: \`${typeName}\`\n\n` +
                `**Fields**:\n${fieldDocs.join('\n')}`);
    ...
    getEnumHover(enumName, document) {
        ...
        if (match) {
            ...
            const markdown = new vscode.MarkdownString(`**Enum**: \`${enumName}\`\n\n` +
                `**Values**:\n${values.map(v => `- \`${v}\``).join('\n')}`);
            return new vscode.Hover(markdown, location.range);
        }
        return new vscode.Hover(new vscode.MarkdownString(`**Enum**: \`${enumName}\``), location.range);
    }
    getFieldHover(fieldName, document) {
        ...
        for (let i = location.range.start.line; i < Math.min(location.range.start.line + 5, lines.length); i++) {
            ...
            if (match && match[1] === fieldName) {
                const type = match[2];
                const markdown = new vscode.MarkdownString(`**Field**: \`${fieldName}\`\n\n` +
                    `**Type**: \`${type}\``);
                return new vscode.Hover(markdown, location.range);
            }
        }
...
exports.ADLHoverProvider = ADLHoverProvider;
